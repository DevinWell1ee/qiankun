<template>
  <div class="layout-wrapper">
    <div class="layout-header">
      ...
      <div class="userinfo">主应用的state：{{ name }}</div>
    </div>
...
export default {
  ...
  computed: {
    name () {
      return store.getGlobalState('name')
    }
  },
  watch: {
    isLoading (val) {
  ...
  methods: {
    ...
    bindCurrent () {
      ...
      const activeApp = this.microApps.find(o => path.includes(o.activeRule))

      console.log('activeApp--', activeApp)
      if (!_.isUndefined(activeApp)) {
        this.current = activeApp.activeRule
      }
    },
